...
var wsServer = new WebSocketServer({ port: 10007 });
...
wsSource.on('message', function (data, flags) {
    if (wsServer.clients.length > 0) { // 有人連進來才處理
        ...
        //if (outputSize.width != 0) {
            ...
            //gm(new Buffer(msg.Img)).gravity('Center').background("#000000").extent(ImgSize.defaultX, ImgSize.defaultY).toBuffer('JPG', function (err, buffer) {
            //gm(new Buffer(msg.Img)).gravity('Center').background("#000000").extent(outputSize.width, outputSize.height).toBuffer('JPG', function (err, buffer) {
